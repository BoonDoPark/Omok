    ...
    public void run() {
        try {
            while (true) {
                ...
                // 사용자의 게임 준비 완료 패킷
                if (receivedPacket.getPacketType().equals(PacketType.PLAYER_READY)) {
                ...
                // 사용자의 돌 좌표 정보 패킷
                if (receivedPacket.getPacketType().equals(PacketType.PLAYER_COORDINATE)) {
                    room.placeStone(receivedPacket);
                }

                if (receivedPacket.getPacketType().equals(PacketType.PLAYER_INITIALIZE)) {
                    room.initializeGame();
                }

                if (receivedPacket.getPacketType().equals(PacketType.PLAYER_EXIT)) {
                    break;
                }
            }
        } catch (IOException e) {
